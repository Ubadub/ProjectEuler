...
object Euler0011 {
  ...
  def productDown(topX: Int, topY: Int): Long = 
    (for (i <- 0 until 4) yield nums(topY + i)(topX)).product.toLong
  
  def productRight(topX: Int, topY: Int): Long = 
    (for (i <- 0 until 4) yield nums(topY)(topX + i)).product.toLong

  def productRightDiagonal(topX: Int, topY: Int): Long =
    (for (i <- 0 until 4) yield nums(topY + i)(topX + i)).product.toLong
  
  def productLeftDiagonal(topX: Int, topY: Int): Long =
    (for (i <- 0 until 4) yield nums(topY + i)(topX - i)).product.toLong

  val maxProductDown: Long = {
    (for (y <- 0 until nums.length - 3;
         x <- 0 until nums.length) 
        yield productDown(x, y)
        ) max
  }

  val maxProductRight: Long =
    (for (y <- 0 until nums.length;
         x <- 0 until nums.length - 3) 
        yield productRight(x, y)).max
  
  val maxProductRightDiagonal: Long =
    (for (y <- 0 until nums.length - 3;
          x <- 0 until nums.length - 3) 
        yield productRightDiagonal(x, y)).max

  val maxProductLeftDiagonal: Long =
    (for (y <- 3 until nums.length - 3;
          x <- 3 until nums.length - 3) 
        yield productLeftDiagonal(x, y)).max
  
  def main(args: Array[String]): Unit = {
    println(maxProductDown)
    println(maxProductRight)
    println(maxProductRightDiagonal)
    println(maxProductLeftDiagonal)
  }
}
